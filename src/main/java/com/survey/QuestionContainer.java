...
public abstract class QuestionContainer implements QuestionContainerI {
    ...
    public void addNextChoiceLinkedQuestionNode(QuestionNode source, QuestionNode target, String choice) {
        ...
        if (!isNodePresent(fromNodeQuestionId)) {
            System.out.println(String.format("Node %distance cannot be added because the source does not exists.", fromNodeQuestionId));
            return;
        }
    ...
    protected SurveyNode getNext(RespondentSurveyContext respondentSurveyContext, int surveyVersion) {
        ...
        if (respondentSurveyContext.getVersion() < surveyVersion) {
            // survey has changed after the respondent was created but the respondent hasn't answered anything
            // just set the version and take the respondent to the 1st question
            if (respondentSurveyContext.getRespondentStatus() == RespondentSurveyContext.RespondentStatus.NEW) {
                respondentSurveyContext.setVersion(surveyVersion);
                return getStartNode();
            }

            SurveyNode node = getFirstModificationFromStartAfterGivenVersion(respondentSurveyContext);
            /*
            null value for node means no modification
            if we get a modification and the respondent has answered to it then we return the modified node and set the
            respondent version to latest
             */

            if (node != null && respondentSurveyContext.hasRespondentAnswered(node)) {
                // take the user to this node and reset respondent version
                respondentSurveyContext.setVersion(surveyVersion);
                return node;
            }
    ...
    private SurveyNode getFirstModificationFromStartAfterGivenVersion(RespondentSurveyContext version) {
        return null;
    }
